"""Lossless bridge from static review cases plus rules-static-v1.json to the IR.

The corpus adapter's premise syntax cannot express recursion, so the static
rule pack is kept in raw IR JSON wire form and merged verbatim.  A case file
contributes facts, assumptions, claims, per-claim diagnostics/mappings and
output templates; the pack contributes every relation declaration and rule.
Nothing here evaluates rules or reads the case's ``expected`` table.
"""
...
STATIC_ROOT = PACKAGE_ROOT / "experiments" / "claim-semantics" / "static"
PACK_PATH = STATIC_ROOT / "rules-static-v1.json"
CASES_DIR = STATIC_ROOT / "cases"
EXPECTED_PATH = STATIC_ROOT / "expected.json"
FROZEN_PRIMITIVES = SRC / "capcov" / "claims" / "static" / "schema_static_v1.json"
...
def load_pack(path: Path = PACK_PATH) -> dict[str, Any]:
    pack = read_json(path)
    if pack.get("schema_version") != 1 or pack.get("id") != "rules-static-v1":
        raise ValueError("unsupported static rule pack")
...
def bundle_payload(case: dict[str, Any], pack: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if case.get("schema_version") != 1 or case.get("rule_pack") != pack["id"]:
        raise ValueError("case does not name the static rule pack")
    ...
    return {"schema_version": 1, "relations": relations, "facts": facts, "evidence": evidence,
            ...
            "metadata": {"case_id": case["id"], "rule_pack": pack["id"], "index": case["context"]["index"],
                         "semantic_inputs": semantic_inputs}}


def load_case(path: Path, pack: dict[str, Any] | None = None) -> Bundle:
    return bundle_from_json(bundle_payload(read_json(path), pack), validate=True)

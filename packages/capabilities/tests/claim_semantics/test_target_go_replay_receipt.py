...
class _JoinCase(unittest.TestCase):
    ...
    def _setup(cls, directory: Path, out_dir: Path) -> None:
        """Judge with every evaluator that is actually here -- never fewer than one.

        ``evaluators="all"`` is python alone in a plain checkout and all three in
        the pinned devShell, so this suite RUNS either way and the differential is
        run wherever it can be.  What is *not* allowed is skipping the judgement
        because an optional tool is absent: the verdicts below are the receipt's,
        not the toolchain's, and ``SOUFFLE_ONLY`` marks the handful of assertions
        that really are about the interpreter.
        """
